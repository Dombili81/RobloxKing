...
try:
    ...
    # torchmcubes derlemesi gerektiriyor; skimage fallback kullan
    ...
    def marching_cubes(volume: torch.Tensor, level: float):
        ...
        verts, faces, _, _ = _skimage_mc(vol_np, level=level, method="lewiner")
        v = torch.from_numpy(verts.copy()).float()
        f = torch.from_numpy(faces.copy().astype(np.int64)).long()
        return v, f
